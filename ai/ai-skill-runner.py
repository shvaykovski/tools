...
def parse_frontmatter(file_path: str) -> Tuple[Dict[str, str], str]:
...
def discover_skills() -> List[Dict[str, object]]:
    ...
    for path in sorted(glob.glob(pattern)):
        ...
        refs_dir: Optional[str] = os.path.join(skill_dir, "references")
        examples_dir: Optional[str] = os.path.join(skill_dir, "examples")
...
class SkillRunner:
    ...
    def list_items(
        ...
        for item in filtered:
            ...
            if len(desc) > 80:
                desc = desc[:77] + "..."
            self.log(f"  {color}{type_str:<8}{RESET} {BOLD}{item['name']:<25}{RESET} {desc}")
    ...
    def auto_route(
        ...
        chosen: str = clean_markdown(chosen_raw).strip().lower() if chosen_raw else "none"

        if chosen == "none" or not chosen:
            self.log(f"   {YELLOW}No matching skill/agent found. Using generic mode.{RESET}")
            return None
        ...
        self.log(f"   {YELLOW}AI suggested '{chosen}' but no match found. Using generic mode.{RESET}")
        return None

    def build_system_prompt(self, item: Dict[str, object]) -> str:
        """Build the system prompt from a skill/agent body, optionally including references.
        ...
        """
        prompt: str = item.get("body", "")

        if self.include_refs:
            # Load references
            for dir_key in ("references_dir", "examples_dir"):
                ref_dir: Optional[str] = item.get(dir_key)
                if ref_dir and os.path.isdir(ref_dir):
                    for ref_file in sorted(glob.glob(os.path.join(ref_dir, "*.md"))):
                        ref_name: str = os.path.basename(ref_file)
                        ref_content: str = read_file_smart(ref_file, max_chars=10000)
                        prompt += f"\n\n### Reference: {ref_name}\n{ref_content}"
    ...
    def run_single(
        ...
        self.log(
            f"{BLUE}🚀 Running {BOLD}{item_name}{RESET} with {BOLD}{self.model}{RESET} "
        ...
        return clean_markdown(response) if response else ""
    ...
    def run_interactive(
        ...
        self.log(f"   Type 'exit' or 'quit' to end. Ctrl+C to abort.\n")
        ...
        while True:
            try:
                ...
                if not response:
                    self.log(f"{RED}No response received.{RESET}")
...
def main() -> None:
    ...
    parser.add_argument("--list", action="store_true", help="List available skills/agents")
    parser.add_argument("--skills", action="store_true", help="Filter listing to skills only")
    parser.add_argument("--agents", action="store_true", help="Filter listing to agents only")
    parser.add_argument(
    ...
    if positional_args:
        # Check if first arg matches a known skill/agent name
        candidate_name: str = positional_args[0]
        # Also check type-qualified names
        is_qualified: bool = ":" in candidate_name and candidate_name.split(":")[0] in (
            "agent",
            "skill",
        )
        name_matches: bool = any(
            it["name"] == candidate_name for it in all_items
        )

        if name_matches or is_qualified:
            # First arg is a skill/agent name
            ...
            user_prompt = " ".join(positional_args)
    else:
        # No positional args
    ...
    if not user_prompt and not stdin_context:
        runner.log(f"{RED}No prompt provided. Pass a prompt or use -i for interactive mode.{RESET}")
        sys.exit(1)
